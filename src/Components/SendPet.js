...
class SendPet extends Component {
    ...
    getSpecialGifs(pet) {
        return axios.get(`https://daily-pet.ru/scripts/get_spec_dates.php?pet=${pet}`).then(res => {
            return res && res.data;
        });
    }
    ...
    render() {
        return (
            <div className="main-content active-send">
                <h2 className="send-header">Send a Pet!<sup>beta</sup></h2>
                <div className="send-content">
                    <form>
                        <p className="top-text">I want to send a funny</p>
                        <div className="form-group form-selector">
                            <label className="pet-selector active"
                                   id="dog-selector"
                                   onClick={e => this.handlePetSelectorClick('dog')}>Dog</label>
                            <span>or</span>
                            <label className="pet-selector"
                                   id="cat-selector"
                                   onClick={e => this.handlePetSelectorClick('cat')}>Cat</label>
                        </div>
                        <p>and here is my gif url:</p>
                        <input type="text"
                               className={'send-input' + (this.state.gifError ? ' error' : '') + (this.state.gif && !this.state.gifError ? ' success' : '')}
                        ...
                        <div className="submit-wrapper">
                            <label className={'submit-button ' + (this.state.selectedDay && this.state.gifError === false ? 'active' : '')}
                                   onClick={e => this.submit()}>Yes yes please!</label>
                        </div>
                    </form>
                </div>
            </div>
